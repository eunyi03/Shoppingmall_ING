...
//       } else {
//         alert("거래완료한 작성자가 아니므로 후기를 등록할 수 없습니다.");
//       }
//     } catch (error) {
...
function DealRead() {
  ...
  const [newReview, setNewReview] = useState(""); // 신규 거래 후기 내용
  const [rating, setRating] = useState(5); // 신규 거래 후기 평점
  const [hasWrittenReview, setHasWrittenReview] = useState(false); // 이미 후기를 작성했는지 여부
      ...
      try {
        ...
        if (response.ok) {
          ...
          const userReview = data.reviews.find(
            (review) => review.CustomerID === currentUserId
          );
          if (userReview) {
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const updatedReviews = await response.json();
        setReviews(updatedReviews.reviews); // 업데이트된 리뷰 목록 반영
        setHasWrittenReview(true); // 후기를 작성한 상태로 변경
      } else if (response.status === 403) {
        const data = await response.json();
        alert(data.message); // "이미 해당 상품에 대한 후기를 작성하셨습니다."
        setHasWrittenReview(true); // 작성 상태로 변경
      } else {
      ...
      {item ? (
        <>
          ...
          <div className="reviews-section">
            ...
            {/* 거래 후기 작성 폼 */}
            {item.Status === "거래완료" && !isOwner && !hasWrittenReview && (
              <div className="review-input">
                <textarea
                  value={newReview}
                  onChange={(e) => setNewReview(e.target.value)}
